...
This service can execute synchronously for local runs or asynchronously behind RunPod Serverless. The same handler powers both entrypoints, so payloads and responses remain identical regardless of execution mode. Unit coverage for these flows lives in [`tests/test_webhook.py`](../tests/test_webhook.py) and [`tests/test_core.py`](../tests/test_core.py).
